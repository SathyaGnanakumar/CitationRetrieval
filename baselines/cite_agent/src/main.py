"""
CiteAgent Baseline Evaluation

LLM-powered citation retrieval agent with search and read capabilities.
...
if metadata["executor"] == "LLMSelfAskAgentPydantic":
    ...
    assert metadata["prompt_name"].endswith(
        "no_search"
    ), "Only no search prompt are supported when executor is LLMNoSearch"
    agent = LLMNoSearch(
        model_name=metadata["model"],
        temperature=metadata["temperature"],
        prompt_name=metadata["prompt_name"],
