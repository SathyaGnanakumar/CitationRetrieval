...
export function CitationFinder() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/find-citation`, {
        method: "POST",
        headers: {
      ...
      if (data.results && data.results.length > 0) {
        ...
        setResult({
          ...
          reasoning: topResult.reasoning || "Citation retrieved from multi-baseline retrieval system combining BM25, E5, and SPECTER retrievers with LLM-based reranking.",
      ...
      setResult({
        citation,
        reasoning: matchedExample.reasoning + " (Note: Using demo data - API unavailable)",
