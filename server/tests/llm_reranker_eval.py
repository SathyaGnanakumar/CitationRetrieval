...
def test_llm_reranker_with_dataset(retriever_name, llm_model, dataset_path, example_idx=0):
    """Test LLM reranker using real dataset and retriever"""

    print("\n" + "=" * 70)
    print(f"Testing LLM-based Reranker with Real Dataset")
    print("=" * 70)
    ...
    # Run LLM reranker
    print(f"\n🤖 Running LLM reranker with model: {llm_model}")
    print("   (This may take a minute depending on your LLM...)")
    result = llm_reranker(state)

    print(f"\n✨ Top 5 papers after LLM reranking:")
    for i, (paper, score) in enumerate(result["ranked_papers"][:5], 1):
        print(f"   {i}. {paper['title'][:80]}...")
        print(f"      Retriever score: {paper['score']:.4f} | LLM score: {score:.4f}")

    # Compare with original ranking
    print(f"\n🔄 Ranking changes:")
    original_titles = [p["title"] for p in candidate_papers[:5]]
    reranked_titles = [paper["title"] for paper, _ in result["ranked_papers"][:5]]

    for i, title in enumerate(reranked_titles, 1):
        original_pos = original_titles.index(title) + 1 if title in original_titles else ">"
        change = ""
        if original_pos != ">":
            if original_pos < i:
                change = f"(↓ from #{original_pos})"
            elif original_pos > i:
                change = f"(↑ from #{original_pos})"
            else:
                change = "(unchanged)"
        print(f"   #{i}: {title[:60]}... {change}")
...
def main():
    parser = argparse.ArgumentParser(description="Test LLM reranker with different retrievers")
    parser.add_argument(
        "--retriever",
        type=str,
        default="bm25",
    ...
    parser.add_argument(
        ...
        type=str,
        default="llama3.2:3b",
        help="LLM model name for reranking (default: llama3.2:3b)",
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    # Run test
    test_llm_reranker_with_dataset(
