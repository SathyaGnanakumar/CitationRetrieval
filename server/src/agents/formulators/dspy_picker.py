...
def dspy_picker(state: Dict[str, Any]):
    ...
    if not isinstance(ranked, list) or not ranked:
        return {"messages": [AIMessage(name="dspy_picker", content="DSPY_PICKER: no ranked_papers")]} 

    query = _get_query(state)
    if not query:
        return {"messages": [AIMessage(name="dspy_picker", content="DSPY_PICKER_ERROR: missing query")]}
    ...
    if not candidates:
        return {"messages": [AIMessage(name="dspy_picker", content="DSPY_PICKER: no usable candidates")]}
    ...
    try:
        ...
        return {
            "messages": [AIMessage(name="dspy_picker", content=f"DSPY_PICKER_ERROR: {e}")]
        }
    ...
    return {
        ...
        "messages": [
            AIMessage(
                name="dspy_picker",
                content=str({"selected_title": selected_title, "selected_id": selected_paper.get("id")}),
            )
        ],
    }
