...
class CitationTrainingExample:
    ...
    def get_candidate_pool(self, shuffle: bool = True) -> List[Dict[str, str]]:
        ...
        """
        candidates = [
            {"title": self.positive_title, "abstract": self.positive_abstract, "is_positive": True}
        ]

        for neg in self.negatives:
            candidates.append(
                {"title": neg["title"], "abstract": neg["abstract"], "is_positive": False}
            )
...
class CitationDataPrep:
    ...
    def __init__(
        self,
        data_path: str = "datasets/scholarcopilot/scholar_copilot_eval_data_1k.json",
    ...
    def _build_negatives(self, bib_info: Dict, exclude_tag: str) -> List[Dict[str, str]]:
        """
        ...
        for tag, refs in bib_info.items():
            ...
            for ref in refs:
                ...
                negatives.append({"title": title, "abstract": abstract or ""})
    ...
    def save_examples(
        self, output_dir: str = "dspy_prompt_generator/data", splits: Optional[Dict] = None
    ):
        """
    ...
    def get_statistics(self) -> Dict:
        ...
        stats = {
            ...
            "examples_with_abstracts": sum(1 for ex in self.examples if ex.positive_abstract),
        }
    ...
    def preview_example(self, idx: int = 0):
        ...
        print(
            f"Abstract: {ex.positive_abstract[:200]}..."
            if ex.positive_abstract
            else "Abstract: N/A"
        )
...
def main():
    ...
    parser.add_argument(
        "--data-path",
        type=str,
        default="datasets/scholarcopilot/scholar_copilot_eval_data_1k.json",
    ...
    parser.add_argument(
        "--max-negatives", type=int, default=10, help="Maximum negatives per example"
    )
    parser.add_argument("--preview", action="store_true", help="Preview examples instead of saving")
